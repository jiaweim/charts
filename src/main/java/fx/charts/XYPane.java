...
import fx.charts.data.XYItem;
import fx.charts.event.ChartEvt;
import fx.charts.event.CursorEvent;
import fx.charts.event.CursorEventListener;
import fx.charts.event.SeriesEventListener;
import fx.charts.series.Series;
import fx.charts.series.XYSeries;
...
import static fx.charts.ChartType.SMOOTH_POLAR;
    ...
    public DoubleProperty averageStrokeWidthProperty() {
        if (null == averageStrokeWidth) {
            averageStrokeWidth = new DoublePropertyBase(_averageStrokeWidth) {
                @Override
                protected void invalidated() {set(Helper.clamp(0.1, 10, get()));}
