...
public class World extends Region {
    ...
    public World(final Resolution RESOLUTION, final Mapping COLOR_MAPPING, final double EVENT_RADIUS, final boolean FADE_COLORS, final OpacityDistribution OPACITY_DISTRIBUTION, final double HEAT_MAP_OPACITY) {
        ...
        fillColor = new StyleableObjectProperty<Color>(FILL_COLOR.getInitialValue(World.this)) {
            @Override
            protected void invalidated() {setFillAndStroke();}
        ...
        locationColor = new StyleableObjectProperty<Color>(LOCATION_COLOR.getInitialValue(this)) {
            ...
            @Override
            public String getName() {return "locationColor";}

            @Override
            public CssMetaData<? extends Styleable, Color> getCssMetaData() {return LOCATION_COLOR;}
        };
        ...
        selectionEnabled = new BooleanPropertyBase(false) {
            @Override
            protected void invalidated() {enableMouseInteraction(get());}
    ...
    private void redraw() {
        if (mapPoints.isEmpty() && mapConnections.isEmpty()) {
            return;
        }
    ...
    private void redrawOverlay() {
        if (null == image || imagePos.length == 0) {
            return;
        }
    ...
    @Override
    public List<CssMetaData<? extends Styleable, ?>> getCssMetaData() {return FACTORY.getCssMetaData();}
