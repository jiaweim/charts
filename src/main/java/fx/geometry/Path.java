...
import fx.geometry.tools.IllegalPathStateException;
import fx.geometry.transform.Affine;
...
public class Path extends Shape {
    ...
    public final void appendOvalQuadrant(double sx, double sy, double cx, double cy, double ex, double ey, double tfrom, double tto, CornerPrefix prefix) {
        if (!(Double.compare(tfrom, 0) >= 0 && Double.compare(tfrom, tto) <= 0 && Double.compare(tto, 1.0) <= 0.0)) {
        ...
        if (prefix == CornerPrefix.MOVE_THEN_CORNER) {
            ...
            if (numTypes == 1 || sx != currentX || sy != currentY) {
                lineTo(sx, sy);
            }
        }
    ...
    @Override
    public Path copy() {return new Path(this);}
        ...
        if (obj instanceof Path) {
            ...
            if (p.numTypes == this.numTypes && p.numCoords == this.numCoords && p.windingRule == this.windingRule) {
                ...
                for (int i = 0; i < numCoords; i++) {
                    if (p.doubleCoords[i] != this.doubleCoords[i]) {
                        return false;
                    }
                }
                return true;
    ...
    @Override
    public final boolean contains(final Point POINT) {return contains(POINT.x, POINT.y);}
    ...
    public void setTo(final Path OTHER_PATH) {
        ...
        if (numCoords > doubleCoords.length) {
            doubleCoords = new double[numCoords];
        }
    ...
    static class CopyIterator extends Path.Iterator {
        ...
        public int currentSegment(final double[] COORDINATES) {
            ...
            if (numCoords > 0) {
                System.arraycopy(doubleCoords, pointIdx, COORDINATES, 0, numCoords);
            }
            return type;
        }
    }
