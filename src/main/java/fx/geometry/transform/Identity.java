package fx.geometry.transform;

import eu.hansolo.toolboxfx.geom.Point;
import fx.geometry.BaseBounds;
import fx.geometry.Path;
import fx.geometry.Rectangle;
import fx.geometry.Shape;
...
public final class Identity extends BaseTransform {
    ...
    @Override
    public Shape createTransformedShape(Shape s) {return new Path(s);}
    ...
    @Override
    public BaseTransform deriveWithTranslation(double mxt, double myt) {return Translate.getInstance(mxt, myt);}

    @Override
    public BaseTransform deriveWithPreTranslation(double mxt, double myt) {return Translate.getInstance(mxt, myt);}
    ...
    @Override
    public BaseTransform deriveWithConcatenation(BaseTransform tx) {return getInstance(tx);}

    @Override
    public BaseTransform deriveWithPreConcatenation(BaseTransform tx) {return getInstance(tx);}

    @Override
    public BaseTransform deriveWithNewTransform(BaseTransform tx) {return getInstance(tx);}

    @Override
    public BaseTransform createInverse() {return this;}

    @Override
    public BaseTransform copy() {return this;}
