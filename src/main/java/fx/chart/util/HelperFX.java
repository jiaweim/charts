...
public class HelperFX {
    ...
    public static Point calcIntersectionPoint(final Point leftPoint, final Point rightPoint, final double intersectionY) {
        double[] xy = calculateIntersectionPoint(leftPoint.getX(), leftPoint.getY(), rightPoint.getX(), rightPoint.getY(), intersectionY);
        return new Point(xy[0], xy[1]);
    }

    public static double[] calculateIntersectionPoint(final Point leftPoint, final Point rightPoint, final double intersectionY) {
        return calculateIntersectionPoint(leftPoint.getX(), leftPoint.getY(), rightPoint.getX(), rightPoint.getY(), intersectionY);
    }

    public static double[] calculateIntersectionPoint(final double x1, final double y1, final double x2, final double y2, final double intersectionY) {
    ...
    public static double getAngleFromXY(final double x, final double y, final double centerX, final double centerY) {
    ...
    public static double[] rotatePointAroundRotationCenter(final double x, final double y, final double rX, final double rY, final double angleDeg) {
    ...
    public static Point getPointBetweenP1AndP2(final Point p1, final Point p2) {
        double[] xy = getPointBetweenP1AndP2(p1.x, p1.y, p2.x, p2.y);
        return new Point(xy[0], xy[1]);
    }

    public static double[] getPointBetweenP1AndP2(final double p1X, final double p1Y, final double p2X, final double p2Y) {
        return new double[]{(p1X + p2X) * 0.5, (p1Y + p2Y) * 0.5};
    }

    public static <T extends Point> List<T> createConvexHull_OLD(final List<T> points) {
        ...
        for (int i = 0; i < points.size(); i++) {
            ...
            if (pointLocation(minPoint, maxPoint, p) == -1) {
                leftSet.add(p);
            } else if (pointLocation(minPoint, maxPoint, p) == 1) rightSet.add(p);
        }
    ...
    public static List<Point> getPointsToCheck(final List<Point> points, final List<Point> hullCurvePoints) {
        ...
        for (Point p : points) {
            for (Bounds area : areas) {
                if (isInRectangle(p.getX(), p.getY(), area.getX(), area.getY(), area.getX() + area.getWidth(), area.getY() + area.getHeight())) {
                    pointsToCheck.add(p);
                }
            }
        }
