...
public class ObservableMatrix<T> {
    ...
    public T getItemAt(final int x, final int y) {
        if (x < 0 || x > (cols.get() - 1) || y < 0 || y > (rows.get() - 1)) {
            throw new IllegalArgumentException("cols/rows cannot be smaller than 0/0 or larger than " + (cols.get() - 1) + "/" + (rows.get() - 1));
        }
    ...
    public boolean contains(final T item) {
        for (int y = 0; y < rows.get(); y++) {
            for (int x = 0; x < cols.get(); x++) {
                if (null != matrix[x][y] && matrix[x][y].equals(item)) {
                    return true;
                }
            }
        }
    ...
    public List<T> getCol(final int col) {
        if (rows.get() == -1 || cols.get() == -1 || col < 0 || col > cols.get()) {
            throw new IllegalArgumentException("cols/rows cannot be smaller 0");
        }
        List<T> c = new ArrayList<>();
        for (int y = 0; y < rows.get(); y++) {
            c.add(matrix[col][y].get());
        }
        return c;
    }
    ...
    public List<T> getRow(final int row) {
        if (rows.get() == -1 || cols.get() == -1 || row < 0 || row > rows.get()) {
            throw new IllegalArgumentException("cols/rows cannot be smaller 0");
        }
        List<T> r = new ArrayList<>();
        for (int x = 0; x < cols.get(); x++) {
            r.add(matrix[x][row].get());
        }
        return r;
    }
    ...
    public void setCols(final int cols, final boolean notify) {
        if (rows.get() == -1 || cols == -1 || this.cols.get() == -1) {
            throw new IllegalArgumentException("cols/rows cannot be smaller 0");
        }
    ...
    public void addRow(final int at, final Supplier<T> itemSupplier, final boolean notify) {
        ...
        for (int x = 0; x < cols.get(); x++) {
            newMatrix[x][at].set(itemSupplier.get());
        }
    ...
    public void addRow(final int at, final List<T> items, final boolean notify) {
        ...
        for (int x = 0; x < cols.get(); x++) {
            newMatrix[x][at].set(items.get(x));
        }
        for (int y = at + 1; y < rows.get(); y++) {
            for (int x = 0; x < cols.get(); x++) {
                newMatrix[x][y].set(matrix[x][y - 1].get());
    ...
    public void setRows(final int rows, final boolean notify) {
        if (rows == -1 || cols.get() == -1 || this.rows.get() == -1) {
            throw new IllegalArgumentException("cols/rows cannot be smaller 0");
        }
    ...
    public List<Integer> getAllEmptyColumns() {
        ...
        for (int x = 0; x < getNoOfCols(); x++) {
            if (getCol(x).stream().filter(Objects::nonNull).count() == 0) {
                emptyColumns.add(x);
            }
        }
        return emptyColumns;
    ...
    public List<Integer> getAllEmptyRows() {
        ...
        for (int y = 0; y < getNoOfRows(); y++) {
            if (getRow(y).stream().filter(Objects::nonNull).count() == 0) {
                emptyRows.add(y);
            }
        }
        return emptyRows;
    ...
    private void checkForRemovedColumnsAndRows(final int removedItemCol, final int removedItemRow, final boolean notify) {
        ...
        for (int r = 0; r < rows.get(); r++) {
            if (null == getItemAt(removedItemCol, r)) {
                nullItemCounter++;
            }
        }
        ...
        for (int c = 0; c < cols.get(); c++) {
            if (null == getItemAt(c, removedItemRow)) {
                nullItemCounter++;
            }
        }
    ...
    public void addMatrixChangeObserver(final EvtType<? extends Evt> type, final EvtObserver<MatrixChangeEvt<T>> observer) {
        ...
        if (null == matrixObservers) {
            matrixObservers = new ConcurrentHashMap<>();
        }
        if (!matrixObservers.containsKey(type)) {
            matrixObservers.put(type, new CopyOnWriteArrayList<>());
        }
    ...
    public void removeMatrixChangeObserver(final EvtType<? extends Evt> type, final EvtObserver<MatrixChangeEvt<T>> observer) {
        if (null == matrixObservers || null == type || null == observer) {
            return;
        }
        if (matrixObservers.containsKey(type)) {
            if (matrixObservers.get(type).contains(observer)) {
    ...
    public void addMatrixItemChangeObserver(final EvtType<? extends Evt> type, final EvtObserver<MatrixItemChangeEvt<T>> observer) {
        ...
        if (null == itemObservers) {
            itemObservers = new ConcurrentHashMap<>();
        }
        if (!itemObservers.containsKey(type)) {
            itemObservers.put(type, new CopyOnWriteArrayList<>());
        }
    ...
    public void removeMatrixItemChangeObserver(final EvtType<? extends Evt> type, final EvtObserver<MatrixItemChangeEvt<T>> observer) {
        if (null == type || null == observer || null == itemObservers) {
            return;
        }
        if (itemObservers.containsKey(type)) {
            if (itemObservers.get(type).contains(observer)) {
