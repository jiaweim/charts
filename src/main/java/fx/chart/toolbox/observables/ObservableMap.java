...
public class ObservableMap<K, V> implements Map<K, V>, Cloneable {
    ...
    @Override
    public V computeIfAbsent(final K key, final Function<? super K, ? extends V> mappingFunction) {return map.computeIfAbsent(key, mappingFunction);}

    @Override
    public V computeIfPresent(final K key, final BiFunction<? super K, ? super V, ? extends V> remappingFunction) {return map.computeIfPresent(key, remappingFunction);}

    @Override
    public V compute(final K key, final BiFunction<? super K, ? super V, ? extends V> remappingFunction) {return map.compute(key, remappingFunction);}

    @Override
    public V merge(final K key, final V value, final BiFunction<? super V, ? super V, ? extends V> remappingFunction) {return map.merge(key, value, remappingFunction);}

    @Override
    public void forEach(final BiConsumer<? super K, ? super V> action) {map.forEach(action);}
    ...
    @Override
    public void replaceAll(final BiFunction<? super K, ? super V, ? extends V> function) {map.replaceAll(function);}
    ...
    @Override
    public boolean equals(final Object obj) {return map.equals(obj);}
    ...
    public void removeMapChangeObserver(final EvtType<? extends Evt> type, final EvtObserver<MapChangeEvt<K, V>> observer) {
        if (null == observers || null == type || null == observer) {
            return;
        }
        if (observers.containsKey(type)) {
            if (observers.get(type).contains(observer)) {
