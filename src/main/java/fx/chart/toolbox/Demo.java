...
import fx.chart.toolbox.Helper.SystemSummary;
...
public class Demo {
    ...
    private void propertiesDemo() {
        ...
        doubleProperty = new DoubleProperty() {
            @Override
            protected void willChange(final Double oldValue, final Double newValue) {
                System.out.println("DoubleProperty will change from " + oldValue + " to " + newValue);
            }

            @Override
            protected void didChange(final Double oldValue, final Double newValue) {
        ...
        integerProperty = new IntegerProperty(10) {
            @Override
            public void set(final int VALUE) {super.set(VALUE);}

            @Override
            public int get() {return super.get();}

            @Override
            protected void didChange(final Integer oldValue, final Integer newValue) {System.out.println("Color changed to: " + newValue);}

            @Override
            public Object getBean() {return Demo.this;}
    ...
    private void stateMachineDemo() {
        ...
        enum MyState implements State {
            ...
            // ******************** Public Methods ************************************
            @Override
            public Set<State> getTransitions() {return this.transitions;}

            @Override
            public boolean canChangeTo(final State state) {return this.transitions.contains(state);}
        ...
        StateMachine<MyState> stateMachine = new StateMachine<>() {
            ...
            @Override
            public void setState(final MyState state) throws StateChangeException {
    ...
    private void evtBusDemo() {
        ...
        class TopicEvt extends ChangeEvt {
            ...
            @Override
            public EvtType<? extends TopicEvt> getEvtType() {return (EvtType<? extends TopicEvt>) super.getEvtType();}
        ...
        class TopicEvtBus implements EvtBus {
            ...
            // ******************** Methods *******************************************
            @Override
            public <T extends Evt> void publish(final Topic topic, final T evt) {
            ...
            @Override
            public void unsubscribe(final Topic topic, final Subscriber subscriber) {
                final EvtType evtType = subscriber.getEvtType();
                if (topicSubscribers.containsKey(topic)) {
                    if (topicSubscribers.get(topic).containsKey(evtType)) {
                        if (topicSubscribers.get(topic).get(evtType).contains(subscriber)) {
                            ...
                            if (topicSubscribers.get(topic).get(evtType).isEmpty()) {
                                topicSubscribers.get(topic).remove(evtType);
                            }
        ...
        class TopicSubscriber implements Subscriber {
            ...
            @Override
            public EvtType<Evt> getEvtType() {return evtType;}

            @Override
            public void handle(final Evt evt) {
