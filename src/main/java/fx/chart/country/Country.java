...
public enum Country {
    ...
    public static List<Country> getAsList() {return Arrays.asList(values());}

    public static Optional<Country> fromIso2(final String iso2) {
        return Arrays.asList(values()).parallelStream().filter(country -> country.name().equalsIgnoreCase(iso2)).findFirst();
    }

    public static Optional<Country> fromName(final String text) {
        return getAsList().parallelStream().filter(country -> country.getDisplayName().equalsIgnoreCase(text)).findFirst();
    }

    public static Map<Country, List<CountryPath>> getCountryPaths() {
        if (null == countryPaths) {
            countryPaths = Helper.createCountryPaths2(Resolution.HI_RES);
        }
        return countryPaths;
    }

    public static Map<Country, List<CountryPath>> getCopyOfCountryPaths() {
    ...
    public static Map<Country, List<Point>> getCountryBounds() {
        if (null == countryBounds) {
            countryBounds = new HashMap<>();
            Properties boundsProperties = Helper.readProperties(Country.class, Constants.BOUNDS_PROPERTIES);
//            final Properties boundsProperties = Helper.readProperties(Constants.BOUNDS_PROPERTIES);
            boundsProperties.forEach((key, value1) -> {
                Optional<Country> country = Country.fromIso2(key.toString());
                if (country.isEmpty()) {
                    return;
                }
                final String[] coords = value1.toString().split(",");
