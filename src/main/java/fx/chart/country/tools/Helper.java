...
public class Helper {
        ...
        if (cities.isEmpty()) {
            try (InputStream in = Helper.class.getResourceAsStream(Constants.CITIES_FILE);
                ...
                lines.forEach(line -> {
                    ...
                    if (null != country) {
                        cities.add(new City(name, lat, lon, country, capital, population));
                    }
                });
                lines.close();
            } catch (IOException e) {
    ...
    public static List<City> getCapitals() {
        if (capitals.isEmpty()) {
            ...
            try (InputStream in = Helper.class.getResourceAsStream(Constants.AIRPORTS_FILE);
                ...
                lines.forEach(line -> {
                    ...
                    if (countryOpt.isPresent()) {
                        airports.put(iata, new Airport(name, lat, lon, countryOpt.get(), size, iata));
                    }
                });
                lines.close();
            } catch (IOException e) {
    ...
    public static Map<String, Airport2> getAirports2() {
            ...
            try (InputStream in = Helper.class.getResourceAsStream(Constants.AIRPORTS2_FILE);
                ...
                lines.forEach(line -> {
                    ...
                    if (airportParts.length == 7) {
                        ...
                        if (countryOpt.isPresent()) {
                            airports2.put(iata, new Airport2(name, lat, lon, countryOpt.get(), iata, icao));
                        }
                    }
                });
                lines.close();
    ...
    public static Map<Country, Long> getPopulations() {
    ...
    public static Map<String, List<CountryPath>> createCountryPaths(final Resolution resolution) {
    ...
    public static Map<Country, List<CountryPath>> createCountryPaths2(final Resolution resolution) {
        final Properties resolutionProperties = readProperties(Country.class, Resolution.HI_RES == resolution ? Constants.HIRES_PROPERTIES : Constants.LORES_PROPERTIES);
    ...
    public static int getMinutes(final double decimalDegree) {return (int) ((decimalDegree - ((int) decimalDegree)) * 60);}

    public static double getSeconds(final double decimalDegree) {return (((decimalDegree - ((int) decimalDegree)) * 60) - getMinutes(decimalDegree)) * 60;}
    ...
    public static Point latLonToXY(final double lat, final double lon) {
    ...
    public static Point latLonToXY(final Point latlon) {
        return latLonToXY(latlon, Constants.MAP_OFFSET_X, Constants.MAP_OFFSET_Y, Constants.MAP_WIDTH, Constants.MAP_HEIGHT);
    }

    public static Point latLonToXY(final Point latlon, final double mapOffsetX, final double mapOffsetY, final double mapWidth, final double mapHeight) {
        final double[] xy = latLonToXY(latlon.getY(), latlon.getX(), mapOffsetX, mapOffsetY, mapWidth, mapHeight);
        return new Point(xy[0], xy[1]);
    }

    public static double[] latLonToXY(final double latitude, final double longitude, final double mapOffsetX, final double mapOffsetY, final double mapWidth, final double mapHeight) {
    ...
    public static Point xyToLatLon(final double x, final double y) {
    ...
    public static Point xyToLatLon(final Point xy) {
        return xyToLatLon(xy, Constants.MAP_OFFSET_X, Constants.MAP_OFFSET_Y, Constants.MAP_WIDTH, Constants.MAP_HEIGHT);
    }

    public static Point xyToLatLon(final Point xy, final double mapOffsetX, final double mapOffsetY, final double mapWidth, final double mapHeight) {
        final double[] ll = xyToLatLon(xy.getY(), xy.getX(), mapOffsetX, mapOffsetY, mapWidth, mapHeight);
        return new Point(ll[0], ll[1]);
    }

    public static double[] xyToLatLon(final double x, final double y, final double mapOffsetX, final double mapOffsetY, final double mapWidth, final double mapHeight) {
    ...
    /**
     * Read a properties file from the class-path
     */
     ...
     * Read a properties file from the class-path
     */
    public static Properties readProperties(Class<?> clz, final String filename) {
        final Properties properties = new Properties();
        try (InputStream resourceStream = clz.getResourceAsStream(filename)) {
            properties.load(resourceStream);
        } catch (IOException exception) {
        }
        return properties;
    ...
    public static void main(String[] args) throws IOException {
        InputStream resourceAsStream = Country.class.getClassLoader().getResourceAsStream("fx/chart/country/bounds.properties");
    ...
    public static Point rotatePointAroundRotationCenter(final Point p, final Point rc, final double angle) {
        double[] rp = rotatePointAroundRotationCenter(p.getX(), p.getY(), rc.getX(), rc.getY(), angle);
        return new Point(rp[0], rp[1]);
    }

    public static double[] rotatePointAroundRotationCenter(final double x, final double y, final double rx, final double ry, final double angle) {
    ...
    public static double getControlPointAngle(final Point p1, final Point p2) {
