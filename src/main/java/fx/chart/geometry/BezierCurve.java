...
public class BezierCurve extends Shape {
    ...
    private static double findZero(double t, final double TARGET, final double EQN[]) {
        ...
        while (true) {
            ...
            if (Double.compare(origdelta, 0) == 0) {
                origdelta = delta;
            }
            if (t < TARGET) {
                if (delta < 0) {
    ...
    private static int getTag(final double COORD, final double LOW, final double HIGH) {
        if (COORD <= LOW) {
            return (COORD < LOW ? BELOW : LOW_EDGE);
        }
        if (COORD >= HIGH) {
            return (COORD > HIGH ? ABOVE : HIGH_EDGE);
        }
        return INSIDE;
    }
    ...
    public boolean intersects(double X, double Y, double WIDTH, double HEIGHT) {
        ...
        for (int i = 0; i < num; i++) {
            tags[i] = getTag(res[i], Y, Y + HEIGHT);
        }
        tags[num] = c1tag;
        Arrays.sort(tags);
    ...
    public boolean contains(double X, double Y) {
        if (!(Double.compare((X * 0.0 + Y * 0.0), 0) == 0)) {
            return false;
        }
    ...
    public boolean contains(double X, double Y, double WIDTH, double HEIGHT) {
        ...
        if (!(contains(X, Y) && contains(X + WIDTH, Y) && contains(X + WIDTH, Y + HEIGHT) && contains(X, Y + HEIGHT))) {
            return false;
        }
    ...
    @Override
    public BezierCurve copy() {return new BezierCurve(x1, y1, ctrlx1, ctrly1, ctrlx2, ctrly2, x2, y2);}
